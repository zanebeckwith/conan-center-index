...
class TestPackageConan(ConanFile):
    ...
    def _skip_test(self):
        ...
        return self.settings.os == "Macos" and self.settings.arch == "armv8" \
               and self.options["oqs-openssl"].shared
    ...
    def build(self):
        if not self._skip_test:
            ...
            openssl_version = tools.Version(self.deps_cpp_info["oqs-openssl"].version)
            if openssl_version.major == "1" and openssl_version.minor == "1":
                cmake.definitions["OPENSSL_WITH_ZLIB"] = False
            else:
                cmake.definitions["OPENSSL_WITH_ZLIB"] = not self.options["oqs-openssl"].no_zlib
            cmake.configure()
            cmake.build()
    ...
    def test(self):
        ...
        assert os.path.exists(os.path.join(self.deps_cpp_info["oqs-openssl"].rootpath, "licenses", "LICENSE"))
