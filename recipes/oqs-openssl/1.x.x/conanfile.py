...
class OpenSSLConan(ConanFile):
    ...
    license = "OpenSSL"
    topics = ("openssl", "ssl", "tls", "encryption", "security", "post-quantum", "liboqs")
    description = "A toolkit for the Transport Layer Security (TLS) and Secure Sockets Layer (SSL) protocols, with post-quantum capabilities provided by liboqs"
    ...
    def configure(self):
        ...
        self.options["liboqs"].use_openssl = False

    def requirements(self):
        if self._full_version < "1.1.0" and self.options.get_safe("no_zlib") == False:
            self.requires("zlib/1.2.11")

        self.requires("liboqs/0.7.1@sbtech/dev") # TODO
...
"""
    ...
    def package_info(self):
        ...
        self.cpp_info.components["crypto"].requires = ["liboqs::liboqs"]
